...
class GalleryContentPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final content = InfiniteScrollView.paginated(
      ...
      onRefresh: () =>
          ref.read(galleryPageAccumulatorProvider(url).notifier).refresh(),
      onLoadMore: () =>
          ref.read(galleryPageAccumulatorProvider(url).notifier).loadNext(),
      hasMore: hasMore,
      ...
      headerItems: [
        ...
        if (hasError)
          HosErrorState(
            ...
            onRetry: () => ref
                .read(galleryPageAccumulatorProvider(url).notifier)
                .refresh(),
          ),
        // 空列表（已加载但无数据）
