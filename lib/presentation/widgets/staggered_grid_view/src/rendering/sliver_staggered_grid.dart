...
class StaggeredGridConfiguration {
  ...
  StaggeredTile? _normalizeStaggeredTile(StaggeredTile? staggeredTile) {
    if (staggeredTile == null) {
      ...
      final crossAxisCellCount = staggeredTile.crossAxisCellCount
      ...
      if (staggeredTile.fitContent) {
        ...
        return StaggeredTile.extent(
          crossAxisCellCount,
          _getStaggeredTileMainAxisExtent(staggeredTile),
        );
      }
    }
  }
...
class RenderSliverStaggeredGrid extends RenderSliverVariableSizeBoxAdaptor {
  ...
  void performLayout() {
    ...
    final pageSize =
        configuration.mainAxisOffsetsCacheSize *
    ...
    final viewportOffsets = _pageSizeToViewportOffsets.putIfAbsent(
      pageSize,
      () => SplayTreeMap<int, _ViewportOffsets?>(),
    );

    _ViewportOffsets? viewportOffset;
    if (viewportOffsets.isEmpty) {
      viewportOffset = _ViewportOffsets(
    ...
    for (
      var index = viewportOffset.firstChildIndex;
      mainAxisOffsets.any((o) => o <= targetEndScrollOffset);
      index++
    ) {
      SliverStaggeredGridGeometry? geometry = getSliverStaggeredGeometry(
        index,
        configuration,
        mainAxisOffsets,
      );
      ...
      if (geometry.trailingScrollOffset >=
        ...
        final nextViewportOffset = _ViewportOffsets(
          mainAxisOffsets,
    ...
    geometry = SliverGeometry(
      ...
      hasVisualOverflow:
          trailingScrollOffset > targetEndScrollOffset ||
  ...
  static SliverStaggeredGridGeometry? getSliverStaggeredGeometry(
    ...
    final geometry = SliverStaggeredGridGeometry(
      ...
      crossAxisExtent:
          configuration.cellStride * tile.crossAxisCellCount -
...
class SliverStaggeredGridDelegateWithFixedCrossAxisCount
  ...
  @override
  bool shouldRelayout(
    covariant SliverStaggeredGridDelegateWithFixedCrossAxisCount oldDelegate,
  ) {
